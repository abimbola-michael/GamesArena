...
class _OnlinePlayersSelectionPageState
  ...
  Future<List<User>> playersToUsers(List<Player> players) async {
    ...
    if (players.isNotEmpty) {
      for (int i = 0; i < players.length; i++) {
        ...
        final user = await fs.getUser(player.id);
        if (user != null) {
          users.add(user);
        }
      }
  ...
  void readUsers() async {
    final user = await fs.getUser(myId);
    if (user != null) {
      selectedUsers.add(user);
    }
    if (!mounted) return;
    ...
    if (group_id != null && group_id != "") {
      players = await fs.readGroupPlayers(group_id!);
    } else {
      players = await fs.readPlayers();
    }
  ...
  void searchUser() async {
    ...
    final searchedUsers = await fs.searchUser(type, value);
